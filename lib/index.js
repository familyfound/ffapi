...
function FFApi(iosocket, settings, cache) {
  this.sock = iosocket
...
extend(FFApi.prototype, {
  get: function (resource, options, next) {
    ...
    this.sock.emit('ffapi', resource, options, function (err, data) {
      if (err) return console.error('failed in ff api', resource, options, err, data)
      if (next) next(data)
    })
  },
  error: function (res) {
